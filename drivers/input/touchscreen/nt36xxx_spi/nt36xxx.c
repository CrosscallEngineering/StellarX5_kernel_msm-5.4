 ...
 *
 * $Revision: 91938 $
 * $Date: 2021-12-06 19:34:27 +0800 (Mon, 06 Dec 2021) $
 *
...
static struct  workqueue_struct *nvt_product_register_wq;
extern void nt36xxx_ts_register_productinfo_wq(struct work_struct *work);
#endif /* CONFIG_HISENSE_PRODUCT_DEVINFO */

struct spi_device *NT_Client = NULL;
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	/* initial customized variable */
	ts->glove_enable = 0;
	ts->resume_is_running = true;

	nt36xxx_factory_ts_func_test_register(ts);
	#ifdef CONFIG_HISENSE_PRODUCT_DEVINFO
		nvt_product_register_wq = alloc_workqueue("nvt_product_register_wq", WQ_UNBOUND | WQ_MEM_RECLAIM, 1);
		if (!nvt_product_register_wq) {
			NVT_ERR("nvt_product_register_wq create workqueue failed\n");
			ret = -ENOMEM;
			goto err_create_nvt_product_register_wq_failed;
		}
		INIT_DELAYED_WORK(&ts->nvt_product_register_work, nt36xxx_ts_register_productinfo_wq);
		queue_delayed_work(nvt_product_register_wq, &ts->nvt_product_register_work, msecs_to_jiffies(16000));
	#else
		nt36xxx_ts_register_productinfo(ts);
	#endif /*CONFIG_HISENSE_PRODUCT_DEVINFO*/
	bTouchIsAwake = 1;
	NVT_LOG("end\n");
...
#ifdef CONFIG_HISENSE_PRODUCT_DEVINFO
	if (nvt_product_register_wq) {
		cancel_delayed_work_sync(&ts->nvt_product_register_work);
		destroy_workqueue(nvt_product_register_wq);
		nvt_product_register_wq = NULL;
	}
err_create_nvt_product_register_wq_failed:
#endif/*CONFIG_HISENSE_PRODUCT_DEVINFO*/
...
int32_t nvt_ts_suspend(struct device *dev)
{
	...
	if ((WAKEUP_GESTURE == 0) || (ts->gesture_enable == false))
		nvt_irq_enable(false);
...
/*
int32_t nvt_glove_enable(uint8_t enable) {
	uint8_t i, retry = 5;
	uint8_t buf[4] = {0};
	int32_t ret = 0;
	...
	for (i=0 ; i<retry ; i++) {
		...
		//---set cmd status---
		buf[0] = EVENT_MAP_HOST_CMD;
		buf[1] = enable ? 0x71 : 0x72;
		CTP_SPI_WRITE(ts->client, buf, 2);

		ts->glove_enable = enable;
		...
		CTP_SPI_READ(ts->client, buf, 2);
		NVT_LOG("buf[1] is %d\n", buf[1]);
		if(buf[1] == enable) { // if((buf[1] >> GLOVE) == enable)
			ts->glove_enable = enable;
			break;
		}
	}
	...
	if (unlikely(i == retry)) {
		NVT_ERR("command failed, buf[1]=0x%02X\n", buf[1]);
		ret = -1;
	}
...
//#if !WAKEUP_GESTURE
	//if((!WAKEUP_GESTURE) || (ts->gesture_enable == false))
	/*delete the judgement to avoid the TP irq  disabled while resume*/
		nvt_irq_enable(true);
//#endif
	...
	if(ts->glove_enable) {
		nvt_set_glove_switch(ts->glove_enable);
	}
#endif
...
static void nvt_resume_work(struct work_struct *work)
{
    struct nvt_ts_data *ts_data = container_of(work, struct nvt_ts_data,
                                  resume_work);
...
extern struct ts_func_test_device *ts_func_test_device[5];
...
static ssize_t att_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size) 
{
	struct ts_func_test_device *test_device;

	printk("echo gesture_en\n");
...
static DEVICE_ATTR(gesture_en, S_IRUGO | S_IWUSR, att_show, att_store);
...
static int att_probe(struct platform_device *ppdev)
{
	...
	dev->kobj = kobject_create_and_add("tp_gesture", NULL); 
	if(dev->kobj == NULL){
		ret = -ENOMEM;
		goto kobj_err;
	}

	ret = sysfs_create_file(&dev->pdev->dev.kobj,&dev_attr_gesture_en.attr);
	if(ret < 0){
		goto file_err;
	}
	return 0;
...
static int att_init(void)
{
	...
	dev->pdev = platform_device_register_simple("tp_gesture", -1, NULL, 0);  
	if(IS_ERR(dev->pdev)){
		printk("%s pdev error\n",__func__);
		return -1;
	}
