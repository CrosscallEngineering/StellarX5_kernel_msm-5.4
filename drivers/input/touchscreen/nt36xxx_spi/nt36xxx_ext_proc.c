 ...
 *
 * $Revision: 83893 $
 * $Date: 2021-06-21 10:52:25 +0800 (Mon, 21 Jun 2021) $
 *
...
int nvt_hs_get_diff(struct nvt_ts_data *data,  char *buf)
{
	int32_t i = 0;
	int32_t j = 0;
	int cunt = 0;
	...
	if (nvt_get_fw_pipe() == 0)
		nvt_read_mdata(data->mmap->DIFF_PIPE0_ADDR, data->mmap->DIFF_BTN_PIPE0_ADDR);
	else
		nvt_read_mdata(data->mmap->DIFF_PIPE1_ADDR, data->mmap->DIFF_BTN_PIPE1_ADDR);
	...
	for (i = 0; i < ts->y_num; i++) {
		for (j = 0; j < ts->x_num; j++) {
			cunt += snprintf(buf + cunt, PAGE_SIZE, "%5d ", xdata[i * ts->x_num + j]);
		}
...
#if TOUCH_KEY_NUM > 0
	for (i = 0; i < TOUCH_KEY_NUM; i++) {
		cunt += snprintf(buf + cunt, PAGE_SIZE, "%5d ", xdata[ts->x_num * ts->y_num + i]);
	}
	cunt += snprintf(buf + cunt, PAGE_SIZE, "\n");
#endif
