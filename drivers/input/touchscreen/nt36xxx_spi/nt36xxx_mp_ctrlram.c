 ...
 *
 * $Revision: 97627 $
 * $Date: 2022-04-06 15:02:09 +0800 (Wed, 06 Apr 2022) $
 *
...
#define SHORT_TEST_CSV_FILE "/storage/emulated/0/FactoryLog/ShortTest.csv"
//#define OPEN_TEST_CSV_FILE "/data/local/tmp/OpenTest.csv"
#define OPEN_TEST_CSV_FILE "/storage/emulated/0/FactoryLog/OpenTest.csv"
//#define FW_RAWDATA_CSV_FILE "/data/local/tmp/FWMutualTest.csv"
#define FW_RAWDATA_CSV_FILE "/storage/emulated/0/FactoryLog/FWMutualTest.csv"
//#define FW_CC_CSV_FILE "/data/local/tmp/FWCCTest.csv"
#define FW_CC_CSV_FILE "/storage/emulated/0/FactoryLog/FWCCTest.csv"
//#define NOISE_TEST_CSV_FILE "/data/local/tmp/NoiseTest.csv"
#define NOISE_TEST_CSV_FILE "/storage/emulated/0/FactoryLog/NoiseTest.csv"
...
static int32_t nvt_save_rawdata_to_csv(int32_t *rawdata, uint8_t x_ch, uint8_t y_ch, const char *file_path, uint32_t offset)
{
	...
	set_fs(KERNEL_DS);
	if (offset == 0){
		#if defined(CONFIG_TOUCHSCREEN_NT36xxx_HOSTDL_IN_QGKI)
...
int32_t nvt_hs_selftest_open(char *buf)
{
	struct device_node *np = ts->client->dev.of_node;
	unsigned char mpcriteria[32] = {0};	//novatek-mp-criteria-default
	int cunt = 0;
	...
	if (TestResult_FW_Rawdata || TestResult_FW_CC || TestResult_Noise || TestResult_Short || TestResult_Open) {
		if (TestResult_FW_Rawdata) {
			cunt += snprintf(buf + cunt, PAGE_SIZE, "RawData_FW_Rawdata test Failed!\n");
			NVT_LOG("RawData_FW_Rawdata test Failed!--\n");
		}
		if (TestResult_FW_CC) {
			cunt += snprintf(buf + cunt, PAGE_SIZE, "RawData_FW_CC test Failed!\n");
			NVT_LOG("RawData_FW_CC test Failed!--\n");
		}
		if (TestResult_Noise) {
			cunt += snprintf(buf + cunt, PAGE_SIZE, "Noise test Failed!\n");
			NVT_LOG("Noise test Failed!--\n");
		}
		if (TestResult_Short) {
			cunt += snprintf(buf + cunt, PAGE_SIZE, "Short test Failed!\n");
			NVT_LOG("Short test Failed!--\n");
		}
		if (TestResult_Open) {
			cunt += snprintf(buf + cunt, PAGE_SIZE, "Open test Failed!\n");
			NVT_LOG("Open test Failed!--\n");
		}
		NVT_LOG("Result Failed!--\n");
		return cunt;
	} else{
		cunt += snprintf(buf, PAGE_SIZE, "All Passed!\n");
		NVT_LOG("All Passed!--\n");
		return cunt;
	}
}

int32_t nvt_hs_get_rawdata(char *buf)
{
	int x=0, y=0, idx = 0;
	struct device_node *np = ts->client->dev.of_node;
	unsigned char mpcriteria[32] = {0};	//novatek-mp-criteria-default
	...
	for (y = 0; y < Y_Channel; y++) {
		for (x = 0; x < X_Channel; x++) {
			idx = y * X_Channel + x;
			cunt += snprintf(buf + cunt, PAGE_SIZE, "%u ", RawData_FW_Rawdata[idx]);
		}
		cunt += snprintf(buf + cunt, PAGE_SIZE, "\n");
	}
