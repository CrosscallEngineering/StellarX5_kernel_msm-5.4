...
struct ts_func_test_device *ts_func_test_device[5];
...
void ts_suspend_for_lcd_async_use(void)
{
	...
	if (test_device && test_device->ts_async_suspend_for_lcd_use)
		test_device->ts_async_suspend_for_lcd_use(test_device->dev);
}
...
void ts_resume_for_lcd_async_use(void)
{
	...
	if (test_device && test_device->ts_async_resume_for_lcd_use)
		test_device->ts_async_resume_for_lcd_use(test_device->dev);
}
...
void ts_reset_for_lcd_use(void)
{
	...
	if (test_device && test_device->ts_reset_for_lcd_use)
		test_device->ts_reset_for_lcd_use(test_device->dev);
}
...
int ts_supend_need_power_reset_high(void)
{
	...
	if (test_device && test_device->ts_suspend_need_lcd_power_reset_high)
		return test_device->ts_suspend_need_lcd_power_reset_high(test_device->dev);
	else
		return false;
}
...
static ssize_t rawdata_cc_show(struct kobject  *obj, struct kobj_attribute *attr, char *buf)
{
	struct ts_func_test_device *test_device;
	int ret = 0;
	int path_num;
	...
	if (test_device && test_device->get_rawdata_cc) {
		ret = test_device->get_rawdata_cc(test_device->dev, buf);
	} else {
		return snprintf(buf, PAGE_SIZE, "%s\n", "NOT SUPPORTED");
	}
	return ret;
...
static ssize_t noise_show(struct kobject  *obj, struct kobj_attribute *attr, char *buf)
{
	struct ts_func_test_device *test_device;
	int ret = 0;
	int path_num;
	...
	if (test_device && test_device->get_noise) {
		ret = test_device->get_noise(test_device->dev, buf);
	} else {
		return snprintf(buf, PAGE_SIZE, "%s\n", "NOT SUPPORTED");
	}
	return ret;
			     ...
			     char *buf)
{
	...
	if (test_device && test_device->get_selftest) {
		ret = test_device->get_selftest(test_device->dev, buf);
		return snprintf(buf, PAGE_SIZE, "%d\n", ret);
	} else
		return snprintf(buf, PAGE_SIZE, "%s\n", "NOT SUPPORTED");
}
...
static struct kobj_attribute rawdata_cc_attr = {
	.attr = {
		.name = "rawdata_ccshow",
		.mode = S_IRUGO,
	},
	.show = rawdata_cc_show,
};

static struct kobj_attribute noise_attr = {
	.attr = {
		.name = "noiseshow",
		.mode = S_IRUGO,
	},
