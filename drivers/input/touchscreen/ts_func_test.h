...
struct ts_func_test_device {
	...
	int (*ts_async_suspend_for_lcd_use)(struct device *dev);
	int (*ts_async_resume_for_lcd_use)(struct device *dev);
	int (*ts_reset_for_lcd_use)(struct device *dev);
	int (*ts_suspend_need_lcd_reset_high)(void);
	int (*ts_suspend_need_lcd_power_reset_high)(struct device *dev);
	...
	int (*get_rawdata_cc)(struct device *dev, char *buf);
	int (*get_noise)(struct device *dev, char *buf);
	int (*get_diff)(struct device *dev, char *buf);
	int (*get_selftest)(struct device *dev, char *buf);
