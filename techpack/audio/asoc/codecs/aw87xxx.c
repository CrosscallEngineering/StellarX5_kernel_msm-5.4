...
//				    aw87xxx_cont->data[i + 1]);
			...
			if (aw87xxx->gain_hold_flag
			    && (aw87xxx_cont->data[i] == AW87XXX_REG_GAIN))
				continue;
...
/*hmct added, for speaker factory*/
int factory_test_result;
extern int msm_anlg_cdc_get_factory_test_spk_on(void);
int aw87xxx_audio_scene_load(uint8_t mode, int32_t channel)
{
    ...
    factory_test_result = msm_anlg_cdc_get_factory_test_spk_on();

  if (((factory_test_result==SPK_1_ON)&&(channel==0))
  	||((factory_test_result==SPK_2_ON)&&(channel==1))
  	||(factory_test_result==SPK_BOTH_ON)){   //hmct added,
    	temp_mode = mode;
	}else{
        temp_mode = AW87XXX_OFF_MODE;
	}
...
{
	int i = 0;
	struct ui_frame_header *frame_header_temp = NULL;
	struct aw87xxx_container *curr_scene_cnt = NULL;
	...
	frame_header_temp = current_scene->frame_header =
	    devm_kzalloc(aw87xxx->dev, sizeof(struct ui_frame_header),
			 GFP_KERNEL);
	if (!frame_header_temp) {
		aw_dev_err(aw87xxx->dev, "%s: Error allocating memory\n",
			   __func__);
		return -ENOMEM;
	}

	memcpy(frame_header_temp, cont->data, sizeof(struct ui_frame_header));

	/*check addr wide, data wide and bin type */
	if (frame_header_temp->addr_bit_wide != AW87XXX_ADDR_BIT_WIDE ||
	    frame_header_temp->data_bit_wide != AW87XXX_DATA_BIT_WIDE ||
	    frame_header_temp->bin_data_type != AW87XXX_BIN_TYPE_REG) {
		aw_dev_err(aw87xxx->dev, "%s: %d bin file type mismatch\n",
			   __func__, frame_header_temp->bin_data_type);
	}

	aw_dev_info(aw87xxx->dev, "%s: bin_fh_ver:0x%08x\n",
		    __func__, frame_header_temp->bin_fh_ver);
	aw_dev_info(aw87xxx->dev, "%s: ui_ver:0x%08x\n",
		    __func__, frame_header_temp->ui_ver);
	aw_dev_info(aw87xxx->dev, "%s: bin_data_ver:0x%08x\n",
		    __func__, frame_header_temp->bin_data_ver);
	aw_dev_info(aw87xxx->dev, "%s: bin_data_type:0x%08x\n",
		    __func__, frame_header_temp->bin_data_type);

	aw87xxx_parse_chip_name(aw87xxx, frame_header_temp->chip_name,
				sizeof(frame_header_temp->chip_name));
	...
	curr_scene_cnt = current_scene->scene_cont =
	    devm_kzalloc(aw87xxx->dev,
			 frame_header_temp->bin_data_len + sizeof(int),
			 GFP_KERNEL);
	if (!curr_scene_cnt) {
		aw_dev_err(aw87xxx->dev, "%s: Error allocating memory\n",
			   __func__);
		if (frame_header_temp != NULL) {
			devm_kfree(aw87xxx->dev, frame_header_temp);
			frame_header_temp = NULL;
		}
		return -ENOMEM;
	}

	curr_scene_cnt->len = frame_header_temp->bin_data_len;
	memcpy(curr_scene_cnt->data,
	       cont->data + sizeof(struct ui_frame_header),
	       frame_header_temp->bin_data_len);
	...
	for (i = 0; i < curr_scene_cnt->len; i = i + 2) {
		...
		if (curr_scene_cnt->data[i] == AW87XXX_REG_GAIN) {
			aw87xxx->raw_gain_array[current_scene->scene_mode] =
			    curr_scene_cnt->data[i + 1];
			aw_dev_info(aw87xxx->dev,
				    "%s: %s_i2c%d@0x%02X mode:%d, raw_gain:%d\n",
				    __func__,
				    aw87xxx_support_product[aw87xxx->product],
				    aw87xxx->i2c_seq, aw87xxx->i2c_addr,
					    ...
					    const struct firmware *cont)
{
	...
	for (i = 0; i < scene_cont->len; i = i + 2) {
		aw_dev_info(aw87xxx->dev, "%s: addr:0x%02X, data:0x%02X\n",
			    ...
			    scene_cont->data[i + 1]);
		if (scene_cont->data[i] == AW87XXX_REG_GAIN) {
			aw87xxx->raw_gain_array[current_scene->scene_mode] =
			    scene_cont->data[i + 1];
			aw_dev_info(aw87xxx->dev,
				    "%s: %s_i2c%d@0x%02X mode:%d, raw_gain:%d\n",
...
static int aw87xxx_actflag_thread(void *pdata)
{
	...
	int active_flag[2] = { 0 };
	int ret;
	static uint32_t thread_off_cnt;
	...
	while (!kthread_should_stop()) {
		if (aw87xxx->current_mode == AW87XXX_OFF_MODE
			...
			ret =
			    aw87xxx_i2c_read(aw87xxx, AW87XXX_REG_GAIN,
					     &reg_val);
			if (ret < 0) {
				pr_err
			...
			if (!err_read_flag && !err_read_reg) {
				...
				if (active_flag[0]) {
					...
					if (reg_val > aw87xxx->actflag_gain_array[aw87xxx->current_mode]) {
						...
						pr_info("%s flag:%d, mode:%d, keep gain:0x%02X!\n",
						     __func__, active_flag[0],
						     aw87xxx->current_mode,
						     reg_val);
					}
