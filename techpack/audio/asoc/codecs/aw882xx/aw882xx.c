...
#ifdef CONFIG_AW_HANDSET_MONITOR_KEY
...
#ifdef CONFIG_AW_HANDSET_MONITOR_KEY
void handset_monitor_select(struct aw882xx *aw882xx, int handset_monitor_key)
{
	if (aw882xx->input) {
		input_report_key(aw882xx->input, handset_monitor_key,1);
		input_sync(aw882xx->input) ;
		msleep(10);
		input_report_key(aw882xx->input, handset_monitor_key,0);
		input_sync(aw882xx->input) ;
...
/*zsq add for handset monitor status start*/
		...
		if (cur_index == 1) {
			...
			{
				handset_monitor_select(aw882xx, HANDSET_MONITOR_OFF_KEY);
				aw_dev_err(aw882xx->dev, "%s zsq add handset off\n", __func__);
			}
			handset_mode_flag = 0;	
		}
#endif
...
/*zsq add for handset monitor status start*/
		...
		if (cur_index == 1) {
			handset_mode_flag = 1;
			handset_monitor_select(aw882xx, HANDSET_MONITOR_ON_KEY);
			aw_dev_err(aw882xx->dev, "%s zsq add handset on\n", __func__);
		}
#endif
	}
...
/*zsq add for handset monitor status start*/
#ifdef CONFIG_AW_HANDSET_MONITOR_KEY
		aw882xx->input = input_allocate_device();
		if (aw882xx->input == NULL) {
			aw_dev_err(aw882xx->dev,"%s: Failed to allocate input\n", __func__);
			ret = -ENOMEM;
			return ret;
		}
		input_set_capability(aw882xx->input , EV_KEY, HANDSET_MONITOR_ON_KEY);
		input_set_capability(aw882xx->input , EV_KEY, HANDSET_MONITOR_OFF_KEY);
		aw882xx->input->name= "HandsetStatus";
		ret = input_register_device(aw882xx->input);
		if (ret) {
			aw_dev_err(aw882xx->dev,"%s: Failed to register input\n", __func__);
			input_free_device(aw882xx->input);
			return ret;
		}
#endif
/*zsq add for handset monitor status end*/
